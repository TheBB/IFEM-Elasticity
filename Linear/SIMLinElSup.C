...
bool SIMLinElSup::parse (const TiXmlElement* elem)
{
  ...
  for (; child; child = child->NextSiblingElement())
    ...
    {
      std::string supId;
      utl::getAttribute(child,"id",supId);
      SIMgeneric* sup = new SIMLinEl3D("3D linear elastic superelement",false);
      if (sup->parse(elem) && !supId.empty())
...
bool SIMLinElSup::recoverInternalDispl (const Vector& glbSol)
{
  ...
  for (SuperElm& sup : mySups)
  {
    ...
    else // No substructure FE model - just use the superlement displacements
      sup.sol = supSol;
  }
...
static bool writeScalarField (const Matrix& field, int geomID,
                             ...
                             double, const char*, int idBlock, int, bool)
{
  ...
  for (size_t pidx = 0; pidx < myModel.size(); pidx++)
  {
    ...
    if (!writeScalarField(field,geomID,nBlock,sID,vtf))
      return -3;
  }
...
bool SIMLinElSup::writeGlvS2 (const Vector& psol, int iStep, int& nBlock,
                              double, int idBlock, int)
{
  ...
  for (size_t pidx = 0; pidx < myModel.size(); pidx++)
  {
    ...
    if (supsim)
      ...
      field.resize(problem->getNoFields(2),myModel[pidx]->getNoNodes()+1);
